...
import { useStudyManagerStore } from "../../store/useStudyManagerStore";
import {
  BookOpen,
  Clock,
  Edit3,
  ...
  if (!activeSemesterId) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-6">
        ...
        <p className="text-text-muted text-lg font-bold italic">
            ...
            {activeSemester && (
              <div className="px-4 py-1 rounded-full bg-neon-cyan/10 border border-neon-cyan/20 flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-text-muted/60">
                  {activeSemester.year} - {activeSemester.type === "ganjil"
                    ? t.profile.ganjil
                    : t.profile.genap}
                </span>
              </div>
        ...
        {filteredCourses.length === 0
          ? (
            <div className="col-span-full py-20 bg-surface-2/30 rounded-3xl border border-dashed border-neon-cyan/10 flex flex-col items-center justify-center gap-4">
              ...
              <p className="text-text-muted/40 font-bold italic tracking-wide">
          ...
          : (
            filteredCourses.map((course) => (
              <div
                key={course.id}
                className="game-panel p-6 flex flex-col gap-6 group hover:border-neon-cyan/40 transition-all duration-300"
              >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {course.type === "Wajib"
                          ...
                          : course.type === "Pilihan"
                          ? t.academic.pilihan
                          : t.academic.mengulang}
                      </span>
                      <span className="text-[10px] font-bold text-text-muted/60 font-mono">
                        {course.code}
                      </span>
                    </div>
                ...
                <div className="flex items-center gap-6 py-3 border-y border-neon-cyan/10">
                  <div className="flex flex-col">
                    <span className="text-[9px] font-black text-text-muted/60 uppercase tracking-widest mb-1">
                      {t.academic.sks}
                    </span>
                    <span className="text-sm font-bold">
                      {course.sks} {t.academic.sks}
                    </span>
                  </div>
            ...
            <div className="flex flex-col gap-8">
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-4">
                  {form.schedules.map((sch, index) => (
                    <div
                      key={index}
                      className="p-6 bg-surface-2/30 rounded-3xl border border-neon-cyan/20 flex flex-col gap-6 relative group/slot shadow-sm"
                    >
              ...
              <button
                className={`btn h-16 w-full text-lg rounded-2xl font-black mt-4 transition-all duration-300 font-display
                  ${
                  !form.name
                    ? "bg-surface-2 text-text-muted cursor-not-allowed border border-neon-cyan/5"
                    : "btn-primary hover:scale-[1.02] shadow-[0_0_30px_rgba(0,240,255,0.15)]"
                }`}
                onClick={handleSave}
                disabled={!form.name}
              >
                {editingId
                  ? t.academic.confirmUpdate
                  : t.academic.acceptArchives}
              </button>
            </div>
          </div>
        </div>
