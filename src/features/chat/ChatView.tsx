      ...
      <div className="flex-1 flex gap-4 md:gap-6 overflow-hidden relative">
        ...
        `}>
              ...
              <div className="flex-1 min-w-0">
                {ui.renaming ? (
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <button onClick={handleRename} className="text-neon-green hover:scale-125 active:scale-90 transition-all"><Check size={14} className="cursor-pointer transition-all" /></button>
                  </div>
                ) : (
          ...
          <div className="flex flex-col gap-3 overflow-y-auto pr-2 flex-1 scrollbar-thin scrollbar-thumb-white/10">
            {users.map(user => (
              <div key={user.id} className="flex flex-col gap-2 p-3 rounded-xl bg-surface-2 border border-neon-cyan/10 group">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs text-white shrink-0 ${user.role === 'admin' ? 'bg-neon-cyan/80 shadow-[0_0_10px_rgba(0,240,255,0.3)]' : 'bg-surface-subtle border border-neon-cyan/10 text-text-muted'}`}>
                      {user.name.charAt(0).toUpperCase()}
                    </div>
                    <div className="min-w-0">
                      <div className="text-xs font-bold flex items-center gap-1.5 truncate">
                        {user.name}
                        {user.role === 'admin' && <ShieldCheck size={10} className="text-neon-cyan" />}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          >
            {messages.length === 0 ? (
              ...
              messages.map((msg, i) => (
                ...
                >
                  {msg.type !== 'text' ? (
                    <div className={`px-4 py-1.5 rounded-full border text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-center font-display ${msg.type === 'event' ? 'bg-neon-cyan/10 text-neon-cyan border-neon-cyan/20' : 'bg-surface-2 text-text-muted border-neon-cyan/10'}`}>
                      {msg.content}
                    </div>
                  ) : (
                    <>
            ...
            {replyingTo && (
              <div className="flex items-center justify-between bg-surface-2 border border-border-main p-3 rounded-t-xl md:rounded-t-2xl border-b-0 animate-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center gap-3 pl-1 md:pl-2 min-w-0">
                   <div className="w-1 h-6 md:h-8 bg-neon-cyan rounded-full shadow-[0_0_8px_rgba(0,240,255,0.4)]"></div>
                   <div className="min-w-0">
                      <div className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-neon-cyan mb-0.5 font-display">Replying to {replyingTo.senderName}</div>
            ...
            <div className="relative">
              ...
              <button 
                className={`absolute right-3 bottom-3 w-10 h-10 rounded-xl flex items-center justify-center transition-all hover:scale-110 active:scale-90 ${
                  form.inputText.trim() ? 'bg-neon-cyan text-white shadow-lg shadow-[0_0_15px_rgba(0,240,255,0.3)]' : 'bg-surface-subtle text-text-muted cursor-not-allowed'
                }`}
                onClick={handleSend}
          ...
          <div className="game-panel w-full max-w-sm p-8 rounded-4xl flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              <button 
                ...
                className={`p-4 rounded-2xl border flex items-center gap-4 transition-all text-left ${fileShare.mode === 'instant' ? 'bg-neon-cyan/10 border-neon-cyan/50 text-neon-cyan' : 'bg-surface-2 border-neon-cyan/10 text-text-muted hover:border-neon-cyan/30'}`}
              >
              ...
              <button 
                ...
                className={`p-4 rounded-2xl border flex items-center gap-4 transition-all text-left ${fileShare.mode === 'on-waiting' ? 'bg-neon-gold/10 border-neon-gold/50 text-neon-gold' : 'bg-surface-2 border-neon-cyan/10 text-text-muted hover:border-neon-cyan/30'}`}
              >
            ...
            {ui.scanning ? (
              <>
                ...
                <div className="absolute inset-0 border-2 border-neon-cyan/50 rounded-3xl pointer-events-none">
                  <div className="absolute top-1/2 left-0 w-full h-0.5 bg-neon-cyan shadow-[0_0_15px_rgba(0,240,255,0.8)] animate-scan-line"></div>
                </div>
              </>
            ) : (
