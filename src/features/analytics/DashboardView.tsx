  ...
  const weeklyStudyTime = useMemo(() => {
    ...
    return sessions
      .filter((s: StudySession) =>
        new Date(s.date).getTime() > now - 86400000 * 7
      )
  ...
  const weekDays = useMemo(() => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push({
        index: i,
        date: d.getDate(),
        dayName: d.toLocaleDateString(language === "id" ? "id-ID" : "en-US", {
          weekday: "short",
        }),
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-8 pb-10 sm:pb-0">
      ...
      {generationState.isActive && (
        <div
          className={`fixed inset-0 z-200 bg-bg-main/90 backdrop-blur-xl flex items-center justify-center p-6 transition-all duration-700 ease-in-out ${
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col gap-6 relative z-10">
              {(generationState.type === "study-plan"
                ? studyPlanSteps
                : googleSyncSteps).map((s, idx) => (
                  <div
                    key={idx}
      ...
      {studyPlan.length > 0 && (
        <div className="game-panel p-6 bg-neon-cyan/5 border-neon-cyan/20 flex flex-col gap-4 animate-in slide-in-from-top-4 duration-500">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xs font-black tracking-[0.2em] font-display text-neon-cyan uppercase">
              {t.dashboard.todayStudyPlan}
            </h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {studyPlan.map((plan, idx) => {
              const course = academicCourses.find((c) =>
                c.id === plan.courseId
              );
              return (
                <div
                  key={idx}
                  className="p-4 rounded-xl bg-surface-2 border border-neon-cyan/10 flex flex-col gap-2"
                >
                  <div className="text-[10px] font-black text-text-muted/40 uppercase tracking-widest">
                    {course?.name}
                  </div>
                  <div className="font-bold text-sm text-text-main line-clamp-1">
                    {plan.topicTitle}
                  </div>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-neon-cyan/5 text-neon-cyan/60 border border-neon-cyan/10">
                      Target: 1h
                    </span>
                  </div>
                </div>
        ...
        <div className="lg:col-span-12 flex flex-col gap-8">
              ...
              <div className="flex items-center gap-4 relative z-10">
                ...
                <div>
                  <p className="font-bold text-text-main italic">
                    <span className="font-black text-[10px] uppercase tracking-widest bg-neon-red/30 text-neon-red px-3 py-1 rounded-lg mr-3 border border-neon-red/30 font-display not-italic flex items-center justify-center gap-1">
                      <AlertCircle size={10} /> {t.dashboard.urgentTask}
                    </span>
          ...
          <div className="game-panel p-8 border-neon-cyan/20 bg-linear-to-br from-[rgba(0,240,255,0.06)] to-[rgba(168,85,247,0.06)] text-text-main flex flex-col gap-6 group hover:border-neon-cyan/40 transition-all duration-300">
            ...
            <div className="flex gap-4 p-6 bg-neon-gold/5 rounded-3xl border border-neon-gold/20 text-sm md:text-base leading-relaxed mt-auto shadow-[0_0_20px_rgba(255,215,0,0.05)]">
              ...
              <p className="font-bold italic text-text-main/80">
                {t.dashboard.tip}
              </p>
            </div>
          </div>
        ...
        <div className="lg:col-span-12">
          <div className="game-panel p-8 flex flex-col gap-8 border-neon-cyan/10">
            ...
            <div className="flex flex-col gap-6">
              <div className="flex justify-between items-end">
                <div className="flex flex-col gap-1">
                  <div className="text-sm font-bold text-text-muted/60">
                    {selectedDateStr}
                  </div>
                  <h2 className="text-4xl font-black neon-cyan-text font-display uppercase tracking-tight">
                    {selectedDayIndex === dayIndex
                      ? "Today"
                      : (language === "id"
                        ? t.academic.days[selectedDayIndex]
                        : weekDays[selectedDayIndex].dayName)}
                  </h2>
                </div>
              </div>
                  ...
                  >
                    ...
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-black transition-all ${
                        d.isToday
                          ? "bg-neon-cyan text-bg-main shadow-[0_0_15px_rgba(0,240,255,0.4)] scale-110"
                          : (selectedDayIndex === i
                            ? "border-2 border-neon-cyan/40 text-neon-cyan"
                            : "text-text-muted/60 hover:bg-surface-2")
                      }`}
                    >
                      {d.date}
                    </div>
            ...
            <div className="flex flex-col gap-0 relative pl-12 mt-4 min-h-50">
              {/* Vertical line */}
              <div className="absolute left-5 top-0 bottom-10 w-0.5 bg-linear-to-b from-neon-cyan/30 via-neon-cyan/10 to-transparent">
              </div>
              ...
              {!activeSemesterId
                ? (
                  <div className="py-20 flex flex-col items-center justify-center gap-4 text-text-muted/20 italic text-center">
                    ...
                    <a
                      href="#/profile"
                      className="text-[10px] text-neon-cyan underline uppercase font-black"
                    >
                      Go to Profile to select
                    </a>
                  </div>
                )
                ...
                ? (
                  <div className="py-20 flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    <p className="font-extrabold tracking-widest uppercase text-sm text-text-main">
                      {language === "id"
                        ? "Hari ini kosong, selamat beristirahat"
                        : "Today is empty, have a good rest"}
                    </p>
                  </div>
                )
                : (
                        ...
                        <div className="game-panel bg-surface-2/40 hover:bg-surface-2 group-hover:border-neon-cyan/30 transition-all duration-300 p-5 md:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 cursor-pointer relative overflow-hidden">
                          <div className="flex flex-col gap-2 flex-1">
                            <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
                              <h4 className="text-lg md:text-xl font-black tracking-tight group-hover:text-neon-cyan transition-colors leading-tight">
                                {sch.courseName}
                              </h4>
                            ...
                            <div className="flex items-center gap-4 text-text-muted/60 text-[10px] md:text-xs font-bold">
                              ...
                              <span className="flex items-center gap-1.5">
                                <User size={14} className="text-neon-cyan/40" />
                                {" "}
                                {sch.lecturer || "N/A"}
                              </span>
                            </div>
