...
import { Calendar, CheckSquare, BookOpen, BarChart3, Menu, X, MessageSquare, Swords, Shield, Gamepad2, Map, ScrollText } from 'lucide-react';
...
export function Layout() {
  ...
  const navItems = [
    { to: "/", icon: <Gamepad2 size={20} />, label: "Command Center", badge: null },
    { to: "/schedule", icon: <Map size={20} />, label: "Mission Timeline", badge: null },
    { to: "/tasks", icon: <ScrollText size={20} />, label: "Quest Board", badge: tasks.filter(t => t.status !== 'done').length || null },
    { to: "/study", icon: <Swords size={20} />, label: "Training Arena", badge: null },
    { to: "/chat", icon: <MessageSquare size={20} />, label: "Guild Hall", badge: null },
  ];
      ...
      <aside className={`app-sidebar ${sidebarOpen ? 'open' : ''} transition-all duration-300 z-50`}>
        <div className="p-6 flex items-center justify-between border-b border-neon-cyan/10">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-black neon-cyan-text font-display tracking-wider">STUDIKU</h1>
          </div>
        ...
        <div className="mx-6 mt-6 p-4 rounded-xl border border-neon-cyan/10 bg-neon-cyan/[0.03] relative overflow-hidden">
          <div className="absolute top-0 right-0 w-20 h-20 bg-neon-cyan/5 rounded-full blur-2xl"></div>
          <div className="flex items-center gap-3 mb-3 relative z-10">
            <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-neon-cyan/20 to-neon-purple/20 border border-neon-cyan/20 flex items-center justify-center">
            ...
            <div>
              ...
              <div className="text-sm font-bold text-text-main">{xp} <span className="text-neon-gold text-[10px] font-display">XP</span></div>
            </div>
          </div>
        ...
        <nav className="p-6 flex flex-col gap-1.5 flex-1 overflow-y-auto">
          <div className="text-text-muted/30 text-[9px] font-black uppercase tracking-[0.25em] px-3 mb-3 font-display">Navigation</div>
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              className={({ isActive }) => `
                ...
                ${isActive 
                  ? 'bg-neon-cyan/[0.08] text-neon-cyan font-bold border border-neon-cyan/20 shadow-[0_0_15px_rgba(0,240,255,0.08)]' 
                  : 'text-text-muted/60 hover:text-text-main hover:bg-white/[0.03] border border-transparent'}
              `}
        ...
        <div className="p-6 border-t border-neon-cyan/10">
          <div className="flex items-center gap-3 px-3 py-2 text-text-muted/30">
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.2em] font-display">System Online</span>
          </div>
        </div>
      </aside>
        ...
        <header className="h-16 px-6 flex items-center justify-between bg-sidebar-bg/80 backdrop-blur-md border-b border-neon-cyan/10 sticky top-0 z-40">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xs font-black tracking-[0.15em] font-display text-text-muted/60 hidden sm:block uppercase">Command Center</h2>
            </div>
          </div>
          <div className="flex items-center gap-3 md:gap-4">
            {/* XP Display */}
            <div className="flex items-center gap-2 px-3 py-1.5 bg-neon-gold/[0.06] rounded-lg border border-neon-gold/15">
              <span className="text-neon-gold text-[10px] font-black font-display">{xp} XP</span>
            </div>
            {/* Season Badge */}
            <div className="px-3 py-1.5 bg-surface-1 rounded-lg border border-neon-cyan/10 text-[10px] font-black shadow-inner flex items-center gap-2 font-display">
              <span className="text-text-muted/40 uppercase tracking-wider hidden xs:inline">Season:</span>
              <span className="text-neon-cyan">S2 2026</span>
            </div>
            {/* Level Badge */}
            <div className="px-3 py-1.5 bg-neon-cyan/[0.06] rounded-lg border border-neon-cyan/15 flex items-center gap-2">
